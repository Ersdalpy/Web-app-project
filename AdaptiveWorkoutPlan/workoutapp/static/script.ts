...
function handleIntensitySelection(workoutType: string, intensity: string) {
    ...
    if (selectionContainer) {
        ...
        if (resultSection) {
            ...
            downloadButton?.setAttribute('href', `${window.location.origin}/static/plans/${fileName}`);
        }
    }
}
...
document.querySelectorAll('.workout-option').forEach(button => {
    (button as HTMLElement).classList.add('hover-scale'); // Add hover-scale class to initial options
